...
import ru.softage.task.crud.db.resopitories.DepartmentRepository;

import static java.lang.String.format;

public class DepartmentsController extends AbstractController<DepartmentModel,
    ...
    void validate(DepartmentModel model){
        ...
        if(model.id != null){
            if(model.id < 0L){
                message += format("ID number must not be non-negative, but id = %id; ", model.id );
            }
        }
    ...
    DepartmentEntity toEntity(DepartmentModel model) {
        return new DepartmentEntity().setId(model.id).setName(model.name).setAddress(model.address);
    }

    @Override
    DepartmentModel toModel(DepartmentEntity entity) {
        return new DepartmentModel().setId(entity.id).setName(entity.name).setAddress(entity.address);
    }

    @GetMapping("/departments/{id}")
    public @ResponseBody
    Response getDepartmentById(@PathVariable("id") Long id) {
