...
import ru.softage.task.crud.db.resopitories.CommonRepository;
import ru.softage.task.crud.db.resopitories.WorkerRepository;
...
@SuppressWarnings("unchecked")
 abstract class AbstractController <M extends AbstractModel, E extends AbstractEntity,
        R extends CommonRepository> {
    ...
    abstract void validate(DepartmentModel model);

    /**
     * @param model specific subclass of{@Link AbstractModel} from request
     * @return DB entity (subclass of {@Link Abstract Entity})
     */
     ...
     * @return specific sublcass of {@Link AbstractModel} for response
     */
    ...
    Response findById(String methodName, Long id) {
        ...
        try {
            ...
            response = new Response().setCode(e.getCode()).setDescription("Format error: "
                    + e.getMessage());
        } catch (ServiceException e) {
            log.error("Processing error:" + e.getMessage());
            response = new Response().setCode(e.getCode()).setDescription("Processing error: "
                    + e.getMessage());
        } catch (Exception e) {
            log.error("Unexpected error: " + e.getMessage());
            response = new Response().setCode(INTERNAL_ERROR).setDescription("Unexpeced error: " +
                    e.getMessage());
        } finally {
    ...
    Response findAll(String methodName) {
        ...
        try {
            ...
            log.error("Unexpected error: " + e.getMessage());
            response = new Response().setCode(INTERNAL_ERROR).setDescription("Unexpected error: " +
                    e.getMessage());
        } finally {
            log.info("response {} -> {}", methodName, response);
        }
    ...
    Response persistOrUpdate(String methodname, M model) {
        ...
        try {
            ...
            response.setDescription(isNewEntity ? "created" : "updated")
    ...
    void validate(Long id){
        if(id != null){
            if(id < 0){
                ...
                throw new ServiceFormatException(format("ID number must be non-negative," +
                        " but id = %d; ", id));
            }
        }
