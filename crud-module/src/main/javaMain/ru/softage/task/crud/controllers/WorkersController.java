...
public class WorkersController extends AbstractController<WorkerModel, WorkerEntity, WorkerRepository> {
    ...
    void validate(WorkerModel model) {
     ...
     if(model.id != null){
        ...
        //resignation can be null

        if(model.departmentId ==null){
            message += "Department ID number is absent; ";
        }else {
            if(model.departmentId < 0L){
                message += format("Department ID must be non-negative, but id = %d; ", model.departmentId);
            }
        }
        if(!message.isEmpty()){
        ...
        log.info("validate successful: {} = {}", model.getClass().getSimpleName(), model);
    }
    ...
    public @ResponseBody
    Response getWorkersById(@PathVariable(name = "id") Long id) {
