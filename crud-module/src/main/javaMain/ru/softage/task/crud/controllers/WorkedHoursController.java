...
import ru.softage.task.crud.db.resopitories.WorkedHoursRepository;
...
@RestController
public class WorkedHoursController extends AbstractController<WorkedHoursModel,
        WorkedHoursEntity, WorkedHoursRepository> {
    ...
    void validate(WorkedHoursModel model) {
        ...
        if (model.workerId == null) {
            ...
            if (model.workerId < 0L) {
                message += format("Worker ID number must be non-negative, but id = %d; ",
                        model.workerId);
            }
        }
    ...
    WorkedHoursModel toModel(WorkedHoursEntity entity) {
        return new WorkedHoursModel().setId(entity.id)
    ...
    public @ResponseBody
    Response getWorkedHoursByWorkerId(@PathVariable(name = "workerId") Long workerId) {
    ...
    public @ResponseBody
    Response getAllWorkedHours() {
        return findAll("getAllWorkedHours");
    }

    @PostMapping("/worked_hours")
    public @ResponseBody
    Response persistOrUpdateWorkedHours(@RequestBody WorkedHoursModel model) {
        return persistOrUpdate("persistOrUpdateWorkedHours", model);
    }

    @DeleteMapping("/worked_hours/{workerId}")
    public @ResponseBody
    Response deleteWorkedHoursByWorkerId(@PathVariable("workerId") Long workerId) {
